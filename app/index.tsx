...
import { Text, TextInput, Button, ActivityIndicator, Card, Divider } from 'react-native-paper';
...
const HomeScreen = () => {
  ...
  const windSpeed = unit === 'imperial' ? weather?.current.wind_mph : weather?.current.wind_kph;
  const humidity = weather?.current.humidity;
  const uvIndex = weather?.current.uv;
  const feelsLike = unit === 'imperial' ? weather?.current.feelslike_f : weather?.current.feelslike_c;
          ...
          <Card style={styles.currentWeatherCard}>
            <Card.Content>
              ...
              <View style={styles.infoRow}>
                <Text variant="bodyLarge">Feels Like:</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {weather.forecast.forecastday[0].hour.map((hourItem, index) => (
              <HourlyWeatherCard
                ...
                wind_speed={unit === 'imperial' ? hourItem.wind_mph : hourItem.wind_kph}
