...
const HomeScreen = () => {
	...
	const weather =
		filteredWeather?.length??0 > 0 ? filteredWeather : day == 0 ? [weatherData?.current] : [dayWeather?.day];
	...
	const precipProbs =
		filteredWeather?.length > 0
			? weather?.map((curr) => curr?.chance_of_rain) ?? []
			: [
					day === 0
			...
			>
				...
				{weatherData && dayWeather && (
					<>
							...
							{expanded ? (
								<>
									{day == 0 && windGusts <= wind + 10 ? (
										<AnimatedInfoRow
											animatedProps={
												animatedWindGustsProps as React.RefAttributes<View>
											}
											label="Wind Gusts"
						...
						<ScrollView
							...
							contentOffset={{
								x:
									day !== 0
										? 152 *
										  (timeOfDaySettings.find((setting) =>
												timeOfDay.includes(setting.label)
										  )?.start ?? 0)
										: 0,
								y: 0,
							}}
						>
							{(day == 0
								? dayWeather?.hour.slice(new Date().getHours())
								: dayWeather?.hour
							).map((hourItem, index) => {
