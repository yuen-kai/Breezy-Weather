	...
	(
		...
		function getDrasticChangeMessage() {
			...
			sortedTimeOfDay = [...timeOfDay]
				...
				.filter((time) => new Date().getHours() < findTimeOfDaySetting(time).end);
			let counter = 0;
			for (const time of sortedTimeOfDay) {
				...
				const values = valuesArray.slice(
					...
					counter +
						Math.max(
							timeOfDaySetting.end - Math.max(timeOfDaySetting.start, new Date().getHours()),
							0
						)
				);
			...
			<View style={styles.infoRow}>
				...
				<View style={{ flex: 1.5, flexDirection: "row", alignItems: "center" }}>
					...
					{drasticChangeMessage && (
						<Tooltip
							popover={
								<Text style={{ color: theme.colors.surface }}>{drasticChangeMessage}</Text>
							}
							visible={open}
							onOpen={() => setOpen(true)}
